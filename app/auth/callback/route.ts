...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Sign in with Supabase using a custom token approach:
    // We store the ORCID session in a cookie directly since we're using
    // ORCID as auth provider outside Supabase Auth.
    ...
    // Store ORCID access token for API calls (shorter lived)
